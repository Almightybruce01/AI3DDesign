...
const { chatCompletionsUrl, chatHeaders, imagesGenerationsUrl, llmConfigured } = require('./openai-compatible');
...
async function enhanceJewelryPrompt(userPrompt, env) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`LLM chat: ${r.status} ${t.slice(0, 200)}`);
  }
  const j = await r.json();
  const out = j.choices?.[0]?.message?.content?.trim();
  return out || userPrompt;
}
