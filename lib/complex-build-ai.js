...
/**
 * GPT-4o: expand a natural-language jewelry brief (+ optional image) into a structured build plan.
 */
...
async function analyzeComplexBrief({ text, imageBuffer, mimeType, env }) {
  ...
  const model = hasImage ? visionModel(env) : complexTextModel(env);
  const userParts = [
    {
      type: 'text',
...
${SCHEMA_HINT}`;
  ...
  const url = hasImage ? openAiVisionChatUrl() : chatCompletionsUrl(env);
  const headers = hasImage ? openAiVisionHeaders(env) : chatHeaders(env);
